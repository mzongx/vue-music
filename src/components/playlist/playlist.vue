        ...
        <scroll-view ref="scroll" :data="sequanceList" class="list-content">
          <transition-group tag="ul" name="list">
            <li 
              ref="listItem" 
              class="item" 
              v-for="(item, index) in sequanceList" 
              :key="item.id" 
            ...
            >
              ...
              <span class="delete" @click.stop="deleteOng(item)">
...
export default {
  ...
  methods: {
    show() {
      this.showFlag = true
      // 因为sequanceList变化的时候，此时playList是隐藏的，所以要在show的时候setTimeout去刷新
      setTimeout(() => {
    ...
    scrollToCurrent(song) {
      let index = this.sequanceList.findIndex((item) => {
